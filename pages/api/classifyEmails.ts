...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const classifiedEmail: ClassifiedEmail = await classifyEmail(email);
    res.status(200).json(classifiedEmail);
    console.log('Classified email');
  } catch (error) {
    console.error('Error classifying email:', error);
    res.status(500).json({ error: 'Failed to classify email', details: (error as Error).message });
  }
}
